...
import { Plus, Trash2, Settings, User, Menu, X, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function Sidebar({ isOpen, onClose, onNewChat, chatHistory = [], onChatClick }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={`
          fixed left-0 top-0 z-50 h-full transform bg-slate-900 text-gray-300 transition-all duration-300 ease-in-out
        ...
        <div className="flex h-full flex-col">
          ...
          <div className="flex items-center justify-between border-b border-slate-800 p-4">
            ...
            {isCollapsed && (
              <button
                onClick={onNewChat}
                className="flex w-full items-center justify-center rounded-lg bg-gradient-to-r from-emerald-600 to-teal-600 p-2.5 text-white transition-colors hover:from-emerald-700 hover:to-teal-700"
                title="Nouveau Chat"
              >
                <Plus className="h-4 w-4" />
              </button>
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={onClose}
                className="rounded-lg p-2 text-gray-400 transition-colors hover:bg-slate-800 hover:text-white lg:hidden"
              >
                <X className="h-5 w-5" />
              </button>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-2">
            {!mounted ? (
              ...
              <div className={`py-8 text-center text-sm text-gray-500 ${isCollapsed ? 'px-2' : 'px-3'}`}>
                {!isCollapsed && 'Aucun historique'}
              </div>
            ) : (
              <>
                {!isCollapsed && todayItems.length > 0 && (
                  <div className="mb-4">
                ...
                {!isCollapsed && olderItems.length > 0 && (
                  <div className="mb-4">
                    <h3 className="mb-2 px-3 text-xs font-semibold uppercase tracking-wider text-gray-500">
                      Plus ancien
                    </h3>
          ...
          <div className="border-t border-slate-800 p-4">
            ...
            {isCollapsed && (
              <div className="flex flex-col items-center gap-2">
                <button className="flex items-center justify-center rounded-lg p-2 text-gray-400 transition-colors hover:bg-slate-800 hover:text-white" title="Utilisateur">
                  <User className="h-4 w-4" />
                </button>
                <button className="flex items-center justify-center rounded-lg p-2 text-gray-400 transition-colors hover:bg-slate-800 hover:text-white" title="Paramètres">
                  <Settings className="h-4 w-4" />
                </button>
              </div>
            )}
