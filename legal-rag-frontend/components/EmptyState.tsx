      ...
      <div className="mx-auto max-w-3xl text-center">
        ...
        <div className="mb-8 flex justify-center">
          <div className="relative animate-float">
            {/* Glow effect */}
            <div className="absolute inset-0 blur-3xl bg-gradient-to-r from-[#0891B2]/20 via-[#14B8A6]/20 to-[#059669]/20 rounded-full scale-150"></div>
            
            {/* Logo container */}
            <div className="relative logo-container h-32 w-32 sm:h-40 sm:w-40 shadow-xl animate-pulse-glow">
              <Image
                src="/assets/logo.png"
        ...
        <div className="mb-6">
          <div className="flex items-center justify-center gap-2 mb-3">
            <h2 className="text-3xl sm:text-4xl font-bold gradient-text-navy">
              Bienvenue sur YoonAssist
            </h2>
            <span className="badge badge-cyan animate-scale-in">
          ...
          {/* Slogan */}
          <p className="text-base sm:text-lg font-semibold gradient-text mb-4">
          ...
          <p className="text-sm sm:text-base text-[#64748B] max-w-xl mx-auto leading-relaxed">
            ...
            Je suis là pour vous aider à comprendre le droit sénégalais.
          </p>
        </div>

        {/* Séparateur décoratif */}
        <div className="flex items-center gap-4 mb-8 justify-center">
          <div className="h-px w-16 bg-gradient-to-r from-transparent to-[#0891B2]/30"></div>
          <span className="text-xs text-[#94A3B8] font-medium uppercase tracking-wider">Questions suggérées</span>
          <div className="h-px w-16 bg-gradient-to-l from-transparent to-[#0891B2]/30"></div>
        </div>
          ...
          {STARTER_QUESTIONS.map((item, index) => {
            ...
            return (
              <button
                ...
                disabled={isLoading}
                className={`
                  group relative flex items-start gap-4 rounded-2xl border-2 p-5 text-left
              ...
              >
                {/* Icône */}
                <div className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-xl ${styles.iconBg} transition-transform group-hover:scale-110`}>
                ...
                <div className="flex-1 min-w-0">
                  {/* Badge catégorie */}
                  <span className={`inline-block text-[10px] font-bold uppercase tracking-wider mb-2 px-2 py-0.5 rounded-full ${styles.badge}`}>
                    {item.category}
                  </span>
                  
                  {/* Question */}
                  <p className="text-sm font-medium text-[#334155] group-hover:text-[#0F2942] leading-relaxed line-clamp-2">
                    {item.question}
                  </p>
                </div>
                
                {/* Flèche */}
                <ArrowRight className={`h-5 w-5 shrink-0 ${styles.icon} opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300`} />
              </button>
            );
          })}
        </div>
        ...
        <p className="mt-8 text-xs text-[#94A3B8]">
          Tapez votre question dans la zone de texte ci-dessous ou cliquez sur une suggestion
        </p>
      </div>
    </div>
  );
